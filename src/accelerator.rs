...
//! ```no_run
//! # use muda_win::accelerator::{Accelerator, Modifiers, Code};
...
//! ```no_run
//! # use muda_win::accelerator::{Accelerator};
...
/// Errors that can occur while parsing an accelerator.
#[derive(Debug)]
pub enum AcceleratorParseError {
    ...
    /// An accelerator should have the modifiers first and only one main key, e.g., "Shift + Alt + K".
...
impl std::fmt::Display for AcceleratorParseError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            AcceleratorParseError::UnsupportedKey(key) => write!(
                f,
                "Couldn't recognize \"{}\" as a valid key for accelerator, if you feel like it should be, please report this to https://github.com/win-rs/muda-win",
                key
            ),
            AcceleratorParseError::EmptyToken(token) => {
                write!(f, "Found empty token while parsing accelerator: {}", token)
            }
            AcceleratorParseError::InvalidFormat(format) => write!(
                f,
                "Invalid accelerator format: \"{}\", an accelerator should have the modifiers first and only one main key, for example: \"Shift + Alt + K\"",
                format
            ),
        }
    }
...
impl StdError for AcceleratorParseError {}
